...
export default function PayWithEsewa() {
  ...
  // [FLOW FEATURE: ESEWA PAYMENT]
  // Auto-submits the form to eSewa once paymentData is generated by the backend API
  useEffect(() => {
    if (paymentData) {
  ...
  // [FLOW FEATURE: ESEWA PAYMENT]
  // Handles package booking via eSewa payment path
  const handleEsewaPayment = async () => {
    try {
      ...
      // Step 1: Parse and validate payment amounts locally before posting
      ...
      // Step 2: Post to the backend to create a pending booking and get signed eSewa fields
      ...
      // Step 3: Set payment data. This triggers the auto-submit Form useEffect block above.
  ...
  // [FLOW FEATURE: BOOK NOW, PAY LATER]
  // Handles booking the package directly, skipping eSewa payment
  const handlePayLater = async () => {
    try {
      ...
      // Step 1: Parse selected date to start date format
      ...
      // Step 2: Call the standard direct booking API (creates pending booking with no transactionUuid)
      ...
      // Step 3: Navigate directly to Trips listing page upon success
      navigate("/trips");
    } catch (err) {
